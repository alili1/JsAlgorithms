...
function yesNo() {
    ...
    let rad = document.getElementsByName('yesNo');
    if (rad[0].checked)
        isolation = true;
}
...
function AntSearch()
{
    ...
    for(let j = 0; j < points.length; j++) //номер муравья
    {
        ...
        if(isolation == true)
        {
            ants[j].len += edges[current][j].len;
        }
    }
...
function pathLength(result) {
    el.value = Math.round(result);
}